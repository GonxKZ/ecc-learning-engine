 ...
 * @brief Comprehensive Educational Examples of Advanced Memory Optimization Techniques
 * 
 * This file contains practical, real-world examples that demonstrate the power and
 * educational value of ECScope's advanced memory management system. Each example
 * is designed to teach specific memory optimization principles while showing
 * measurable performance improvements.
 * 
 * Examples covered:
 * 1. NUMA-Aware ECS Component Management
 * 2. Cache-Line Optimized Data Structures
 * 3. Thread-Local Memory Pool Performance
 * 4. Memory Bandwidth Analysis and Optimization
 * 5. Lock-Free Allocator Comparison
 * 6. False Sharing Prevention Techniques
 * 7. Hot/Cold Data Separation Benefits
 * 8. Memory Access Pattern Impact Analysis
 * 
 * @author ECScope Educational ECS Framework - Memory Optimization
 * @date 2025
 */
...
void demonstrate_bandwidth_analysis() {
    ...
    // Get available NUMA nodes for testing
    auto& numa_manager = numa::get_global_numa_manager();
    auto available_nodes = numa_manager.get_topology().get_available_nodes();
    
    std::cout << "Running bandwidth profiling on " << available_nodes.size() << " NUMA nodes...\n";
    
    // Test sequential access patterns across all nodes
    for (u32 node : available_nodes) {
        auto measurements = bandwidth_profiler.profile_all_patterns(node);
        
        f64 best_bandwidth = 0.0;
        for (const auto& measurement : measurements) {
            best_bandwidth = std::max(best_bandwidth, measurement.get_effective_bandwidth());
        }
        
        std::cout << "Node " << node << " peak bandwidth: " << std::fixed << std::setprecision(2)
                  << best_bandwidth << " GB/s\n";
    }
    
    // Generate comprehensive profiling report
    std::cout << "\nBandwidth Profiling Report:\n";
    auto profiling_report = bandwidth_profiler.generate_comprehensive_report();
    std::cout << profiling_report.summary << "\n";
    ...
    if (bottlenecks.empty()) {
        ...
        for (const auto& bottleneck : bottlenecks) {
            std::cout << "  - " << bottleneck.bottleneck_type 
                      << ": Impact " << std::fixed << std::setprecision(1)
                      << bottleneck.impact_score * 100 << "%\n";
    ...
    std::cout << "Performance Optimization Recommendations:\n";
    auto recommendations = bottleneck_detector.generate_optimization_recommendations();
    std::cout << recommendations.summary;
}
